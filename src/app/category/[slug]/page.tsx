...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  const categoryData = await fetchPayload<PaginatedDocs<Category>>(
  ...
  const [postsData, allCategoriesData] = await Promise.all([
    fetchPayload<PaginatedDocs<Post>>(
      `/api/posts?sort=-createdAt&limit=9&page=${page}&depth=2${queryFilter}`
    ),
